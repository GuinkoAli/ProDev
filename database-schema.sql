...
BEGIN
  ...
  IF NOT poll_record.is_public THEN
    RAISE EXCEPTION 'Poll is not public';
  END IF;
  ...
  IF NOT poll_record.allow_multiple_votes THEN
    SELECT id INTO existing_vote FROM public.votes WHERE poll_id = poll_uuid AND voter_id = voter_uuid LIMIT 1;
    IF FOUND THEN
